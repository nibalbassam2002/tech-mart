        ...
        <form method="POST" action="{{ route('admin.products.update', $product->id) }}" enctype="multipart/form-data">
                ...
                <div class="col-md-8">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="main_category" class="form-label required-field">{{ __('admin.main_category') }}</label>
                            <select id="main_category" class="form-select" required>
                                <option value="">-- {{ __('admin.select') }} --</option>
                                @foreach ($mainCategories as $cat)
                                <option value="{{ $cat->id }}" {{ old('main_category', optional($mainCategory)->id) == $cat->id ? 'selected' : '' }}>
                                    {{ $cat->name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="category_id" class="form-label required-field">{{ __('admin.sub_category') }}</label>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <select name="currency" class="form-select" style="max-width: 100px;">
                                ...
                                <option value="JOD" {{ old('currency', $product->currency) == 'JOD' ? 'selected' : '' }}>JOD</option>
                            </select>
                        </div>
                    </div>
                    <div class="mb-3">
            ...
            <div class="row" id="media-gallery">
                ...
                <div class="col-md-3 text-center position-relative mb-3" id="media-container-{{ $media->id }}">
                    ...
                    @endif
                    <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0 m-1 delete-media-btn" data-media-id="{{ $media->id }}"><i class="bi bi-x-lg"></i></button>
                </div>
                @empty
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchAttributes(subCatId) {
        if (!subCatId) {
            attributesContainer.innerHTML = '<p class="text-muted text-center">Select a sub-category to see its attributes.</p>';
            return;
        }
        ...
        fetch(`/admin/get-attributes/${subCatId}`)
            ...
            .then(data => {
                ...
                if (data.length === 0) {
                    attributesContainer.innerHTML = '<p class="text-muted text-center">No attributes found for this category.</p>';
                } else {
                    data.forEach(attr => {
                        let options = '';
                        attr.values.forEach(val => {
                            // Check if the current value's ID is in the product's associated values
                            const isSelected = currentAttributeValues.includes(val.id) ? 'selected' : '';
                            options += `<option value="${val.id}" ${isSelected}>${val.value}</option>`;
                        });
                        const attrHtml = `
            ...
            .catch(error => {
                ...
                attributesContainer.innerHTML = '<p class="text-danger text-center">Failed to load attributes.</p>';
            });
    }
